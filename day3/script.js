    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        for (let j = 0; j < row.length; j++) {
            ...
            if (Number(char) || char === '0') {
                let k = j + 1;
                let number = char;
                while (Number(row[k]) || row[k] === '0') {
                    number += row[k];
                    k++;
                }
                if(lookAtAdjacentCells(arr, i, j, k)) count += Number(number);
                j = k - 1;
            }
        }
    }
...
function lookAtAdjacentCells(arr, row, start, end) {
    for (let i = row - 1; i <= row + 1; i++) {
        if (i === -1 || i === arr.length) continue;
        const str = arr[i];
        for (let j = start - 1; j <= end; j++) {
            if (j === -1 || j >= str.length) continue;
            const cur = str[j];
            if (!Number(cur) && cur !== '.') return true;
        }
    }
    return false;
}
